...
def apply_theme(active_page: str = "Home"):
    ...
    <style>
        ...
        /* HIDE DEFAULT STREAMLIT SIDEBAR & COLLAPSE CONTROL TO USE TOP NAVBAR */
        ...
        .nav-btn-active>button {
            ...
            box-shadow: 0 0 14px rgba(228, 255, 91, 0.25) !important;
            padding: 8px 12px !important;
            font-size: 0.88rem !important;
            font-weight: 700 !important;
            border-radius: 10px !important;
    ...
    with st.container():
        ...
        col_logo, col_h, col_hw, col_f, col_p, col_s, col_g, col_cta = st.columns([1.8, 1, 1.2, 1, 1.1, 1, 1.2, 1.3], gap="small")
        
        with col_logo:
            st.markdown("""
            <div style="font-family: 'Space Grotesk', sans-serif; font-weight: 800; font-size: 1.25rem; color: #fff; padding-top: 6px;">
                ⚡ <span style="background: linear-gradient(90deg, #fff, #E4FF5B); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">StudentFit AI</span>
            </div>
            """, unsafe_allow_html=True)

        with col_h:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "Home" else "nav-btn"}">', unsafe_allow_html=True)
            if st.button("🏠 Home", key="nav_btn_home", use_container_width=True):
                st.switch_page("pages/home.py")
            st.markdown('</div>', unsafe_allow_html=True)

        with col_hw:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "How it Works" else "nav-btn"}">', unsafe_allow_html=True)
            if st.button("📖 How it Works", key="nav_btn_how", use_container_width=True):
                st.switch_page("pages/how_it_works.py")
            st.markdown('</div>', unsafe_allow_html=True)

        with col_f:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "Features" else "nav-btn"}">', unsafe_allow_html=True)
            if st.button("✨ Features", key="nav_btn_feat", use_container_width=True):
                st.switch_page("pages/features.py")
            st.markdown('</div>', unsafe_allow_html=True)

        with col_p:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "Plans" else "nav-btn"}">', unsafe_allow_html=True)
            if st.button("🏷️ Plans", key="nav_btn_plans", use_container_width=True):
                st.switch_page("pages/plans.py")
            st.markdown('</div>', unsafe_allow_html=True)

        with col_s:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "Story" else "nav-btn"}">', unsafe_allow_html=True)
            if st.button("💡 Story", key="nav_btn_story", use_container_width=True):
                st.switch_page("pages/story.py")
            st.markdown('</div>', unsafe_allow_html=True)

        with col_g:
            st.markdown(f'<div class="{"nav-btn-active" if active_page == "AI Generator" else "nav-btn"}">', unsafe_allow_html=True)
